...
const overviewItem: { key: AdminPanelKey; icon: string; label: string } = {
  key: "overview",
  icon: "📊",
  label: "Tổng quan",
};

const navGroups: { id: string; label: string; items: { key: AdminPanelKey; icon: string; label: string }[] }[] = [
  {
    id: "content",
    label: "Chiến dịch & nội dung",
    items: [
      { key: "campaigns", icon: "🎯", label: "Duyệt chiến dịch" },
      { key: "news", icon: "📰", label: "Bài viết / Tin tức" },
    ],
  },
  {
    id: "verification",
  ...
  },
  {
    ...
    items: [
      ...
      { key: "wallet", icon: "💳", label: "Đối soát nạp ví" },
      { key: "disbursement", icon: "💰", label: "Hậu kiểm giải ngân" },
    ],
  },
  {
    id: "operations",
    label: "Điều phối & cộng đồng",
    items: [
...
export function AdminSidebar({ active, labels, onSelect }: AdminSidebarProps) {
  ...
  const activeGroup = navGroups.find((group) => group.items.some((item) => item.key === active))?.id;
  ...
  function renderItem(item: { key: AdminPanelKey; icon: string; label: string }) {
  ...
  return (
    <aside className="flex flex-col bg-chamDeep px-3 py-5 text-white lg:min-h-screen lg:px-0 lg:py-6">
      <div className="border-b border-white/10 px-2 pb-4 lg:px-5">
        <div className="text-sm text-white/65">Thiện Nguyện</div>
        <strong className="font-serif text-[17px]">Admin Portal</strong>
      </div>

      <nav className="flex gap-1 overflow-x-auto pt-3 lg:flex-col lg:gap-1 lg:overflow-visible lg:px-0">
        <div className="flex gap-1 lg:block">{renderItem(overviewItem)}</div>
            ...
            return (
              <section key={group.id} className="mt-2">
                <button
                  ...
                  className="flex w-full items-center justify-between px-5 py-2 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-white/45 transition hover:text-white/75"
                >
                  {group.label}<span aria-hidden className={`text-[10px] transition-transform ${isOpen ? "rotate-180" : ""}`}>⌄</span>
                </button>
                {isOpen ? <div>{group.items.map(renderItem)}</div> : null}
              </section>
            );
          })}
        </div>
        <div className="flex gap-1 lg:hidden">{navGroups.flatMap((group) => group.items).map(renderItem)}</div>
      </nav>
