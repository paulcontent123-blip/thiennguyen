...
export function OrganizationDashboard({ organization, campaigns, page, total, totalPages }: { organization: Organization; campaigns: Campaign[]; page: number; total: number; totalPages: number }) {
    ...
    <section className="mx-auto max-w-[1160px] px-6 py-10">
      ...
      <div className="mt-8 overflow-hidden rounded-[12px] border border-line bg-white">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-sm"><thead className="bg-paper text-left text-xs uppercase tracking-wide text-inkMid"><tr><th className="px-4 py-3">Chiến dịch</th><th className="px-4 py-3">Loại</th><th className="px-4 py-3">Mục tiêu</th><th className="px-4 py-3">Thời hạn</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3">Thao tác</th></tr></thead><tbody>
          {campaigns.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center text-inkSoft">Chưa có chiến dịch nào.</td></tr> : campaigns.map((campaign) => <tr key={campaign.id} className="border-t border-line align-top">
            <td className="px-4 py-3"><div className="font-bold text-chamDeep">{campaign.title}</div><div className="mt-1 text-xs text-inkSoft">{campaign.category ?? "Chưa phân loại"} · tạo {date.format(new Date(campaign.created_at))}</div>{campaign.review_note ? <p className="mt-2 text-xs text-son">Admin: {campaign.review_note}</p> : null}</td>
            <td className="px-4 py-3 text-inkMid">{campaign.campaign_type === "direct" ? "Trực tiếp" : "Kết nối"}</td><td className="px-4 py-3 font-mono font-bold text-son">{currency.format(Number(campaign.target_amount) || 0)}đ</td><td className="px-4 py-3 text-inkMid">{campaign.deadline ? date.format(new Date(campaign.deadline)) : "Không giới hạn"}</td><td className="px-4 py-3"><CampaignPill status={campaign.status} /></td>
            <td className="px-4 py-3">{["draft", "needs_revision"].includes(campaign.status) ? <button type="button" disabled={busy !== null || !canCreateCampaign} onClick={() => runAction(`campaign-${campaign.id}`, () => submitCampaignForReview(campaign.id), setCampaignNotice)} className="rounded-[6px] bg-chamDeep px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-45">{busy === `campaign-${campaign.id}` ? "Đang gửi…" : "Gửi duyệt"}</button> : <span className="text-xs text-inkSoft">—</span>}</td>
          </tr>)}</tbody></table></div>
